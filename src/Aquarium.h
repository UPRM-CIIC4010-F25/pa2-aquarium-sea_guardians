...
enum class AquariumCreatureType {
    ...
    PlaneFish,
    RainbowFish,
    Jellyfish,      // new: stinging/obstacle creature
    Shark,          // new: fast predator
    CleanerFish,    // new: friendly, gives small benefits
    Coral           // new: stationary obstacle
};
...
public:
    ...
    std::shared_ptr<GameSprite> m_cleaner_fish;
    std::shared_ptr<GameSprite> m_coral;
};
...
public:
    ...
    //itemBlock
    void scheduleNextItemBlock(float now);
    void spawnItemBlock();
    void updateItemBlock(float now, float dt);
    void drawItemBlock();
    void checkItemBlockCollision(float now, std::shared_ptr<PlayerCreature> player);
    void onScoreUpdated(int score, float now);


    PowerUpBox itemBlock;
    bool itemBlockPresent = false;

    float getScoreMultiplier() const;
    float getScoreMultRemaining(float now) const;
    ...
    public:
        Level_0(int levelNumber, int targetScore): AquariumLevel(levelNumber, targetScore){
            this->m_levelPopulation.push_back(std::make_shared<AquariumLevelPopulationNode>(AquariumCreatureType::NPCreature, 5));
             this->m_levelPopulation.push_back(std::make_shared<AquariumLevelPopulationNode>(AquariumCreatureType::PlaneFish, 2));
    ...
    public:
        Level_2(int levelNumber, int targetScore): AquariumLevel(levelNumber, targetScore){
            ...
            this->m_levelPopulation.push_back(std::make_shared<AquariumLevelPopulationNode>(AquariumCreatureType::BiggerFish, 5));
             this->m_levelPopulation.push_back(std::make_shared<AquariumLevelPopulationNode>(AquariumCreatureType::RainbowFish, 1));
    this->m_levelPopulation.push_back(std::make_shared<AquariumLevelPopulationNode>(AquariumCreatureType::PlaneFish, 7));
